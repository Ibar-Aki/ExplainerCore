...
function buildBreakdown(
  ...
  const fillerPenalty =
    (transcript.includes('えっと') ? 0.6 : 0) +
    (transcript.includes('あの') ? 0.4 : 0) +
    (transcript.includes('その') ? 0.2 : 0);
...
export function createFallbackEvaluation(
  ...
  const breakdown = buildBreakdown(rubric, transcript, request.answerPreparationSec);
  const weightedScore =
  ...
  const selfCheckComparison = createSelfCheckComparison(request.baselineSelfCheck, overallScore);
